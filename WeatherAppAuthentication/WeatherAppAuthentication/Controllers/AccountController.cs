...
using WeatherAppAuthentication.ViewModels;

namespace WeatherAppAuthentication.Controllers;
...
public class AccountController : ControllerBase
{
    ...
    public async Task<IActionResult> GetUserByUserName(string userName)
    {
        ...
        if (!(await _authorizationService.AuthorizeAsync(User,
                appUser?.Id ?? string.Empty, AccountManagementOperations.Read))
            .Succeeded)
            return new ChallengeResult();
    ...
    {
        var usersAndRoles =
            await _accountManager.GetUsersAndRolesAsync(pageNumber, pageSize);
    ...
    public async Task<IActionResult> UpdateCurrentUser(
        [FromBody] UserEditViewModel user)
    {
        ...
        [FromBody] UserEditViewModel user)
    {
        ...
        var currentRoles = appUser != null
            ? (await _accountManager.GetUserRolesAsync(appUser)).ToArray()
            : null;

        var manageUsersPolicy =
            _authorizationService.AuthorizeAsync(User, id,
                AccountManagementOperations.Update);
        var assignRolePolicy = _authorizationService.AuthorizeAsync(User,
            (user.Roles, currentRoles), Policies.AssignAllowedRolesPolicy);

        if ((await Task.WhenAll(manageUsersPolicy, assignRolePolicy)).Any(r =>
                !r.Succeeded))
            return new ChallengeResult();
        ...
        if (ModelState.IsValid)
        {
            ...
            if (!string.IsNullOrWhiteSpace(user.Id) && id != user.Id)
                return BadRequest(
                    "Conflicting user id in parameter and model data");
            ...
            var isUserNameChanged = !appUser.UserName.Equals(user.UserName,
            ...
            if (Utilities.GetUserId(User) == id)
            {
                if (string.IsNullOrWhiteSpace(user.CurrentPassword))
                {
                    ...
                    if (isUserNameChanged)
                        AddError(
                            "Current password is required when changing your own username",
                            "Username");
                }
                else if (isPasswordChanged || isUserNameChanged)
                {
                    if (!await _accountManager.CheckPasswordAsync(appUser,
            ...
            if (ModelState.IsValid)
            {
                ...
                var result =
                    await _accountManager.UpdateUserAsync(appUser, user.Roles);
                if (result.Succeeded)
                {
                    if (isPasswordChanged)
                    {
                        if (!string.IsNullOrWhiteSpace(user.CurrentPassword))
                            result =
                                await _accountManager.UpdatePasswordAsync(
                                    appUser, user.CurrentPassword,
                                    user.NewPassword);
                        else
                            result =
                                await _accountManager.ResetPasswordAsync(
                                    appUser, user.NewPassword);
                    }
    ...
    [ProducesResponseType(400)]
    public async Task<IActionResult> UpdateCurrentUser(
        [FromBody] JsonPatchDocument<UserPatchViewModel> patch)
    {
    ...
    {
        if (!(await _authorizationService.AuthorizeAsync(User, id,
                AccountManagementOperations.Update)).Succeeded)
            return new ChallengeResult();
    ...
    public async Task<IActionResult> Register([FromBody] UserEditViewModel user)
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            var result =
                await _accountManager.CreateUserAsync(appUser, user.Roles,
                    user.NewPassword);
            if (result.Succeeded)
            {
                var userVM = await GetUserViewModelHelper(appUser.Id);
                return CreatedAtAction(GetUserByIdActionName,
                    new {id = userVM.Id}, userVM);
            }
    ...
    {
        if (!(await _authorizationService.AuthorizeAsync(User, id,
                AccountManagementOperations.Delete)).Succeeded)
            return new ChallengeResult();
        ...
        if (!await _accountManager.TestCanDeleteUserAsync(id))
            return BadRequest(
                "User cannot be deleted. Delete all orders associated with this user and try again");
        ...
        if (!result.Succeeded)
            throw new Exception(
                $"The following errors occurred whilst deleting user: {string.Join(", ", result.Errors)}");
    ...
    public async Task<IActionResult> UnblockUser(string id)
    {
        ...
        if (!result.Succeeded)
            throw new Exception(
                $"The following errors occurred whilst unblocking user: {string.Join(", ", result.Errors)}");
    ...
    public async Task<IActionResult> UserPreferences([FromBody] string data)
    {
        ...
        if (!result.Succeeded)
            throw new Exception(
                $"The following errors occurred whilst updating User Configurations: {string.Join(", ", result.Errors)}");
    ...
    {
        var roles =
            await _accountManager.GetRolesLoadRelatedAsync(pageNumber,
                pageSize);
        ...
        [FromBody] RoleViewModel role)
    {
        if (ModelState.IsValid)
        {
            ...
            if (!string.IsNullOrWhiteSpace(role.Id) && id != role.Id)
                return BadRequest(
                    "Conflicting role id in parameter and model data");
            ...
            var result = await _accountManager.UpdateRoleAsync(appRole,
                role.Permissions?.Select(p => p.Value).ToArray());
            if (result.Succeeded)
                return NoContent();
    ...
    public async Task<IActionResult> CreateRole([FromBody] RoleViewModel role)
    {
        if (ModelState.IsValid)
        {
            ...
            if (result.Succeeded)
            {
                ...
                return CreatedAtAction(GetRoleByIdActionName,
                    new {id = roleVM.Id}, roleVM);
            }
    ...
    public async Task<IActionResult> DeleteRole(string id)
    {
        ...
        if (!await _accountManager.TestCanDeleteRoleAsync(id))
            return BadRequest(
                "Role cannot be deleted. Remove all users from this role and try again");
        ...
        if (!result.Succeeded)
            throw new Exception(
                $"The following errors occurred whilst deleting role: {string.Join(", ", result.Errors)}");
    ...
    {
        return Ok(
            _mapper.Map<List<PermissionViewModel>>(ApplicationPermissions
                .AllPermissions));
    }
