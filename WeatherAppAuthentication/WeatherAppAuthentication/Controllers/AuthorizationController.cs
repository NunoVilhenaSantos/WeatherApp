...
public class AuthorizationController : Controller
{
    ...
    public async Task<IActionResult> Exchange()
    {
        ...
        if (request.IsPasswordGrantType())
        {
            ...
            var principal =
                await CreateClaimsPrincipalAsync(user, request.GetScopes());
        ...
        throw new InvalidOperationException(
            $"The specified grant type \"{request.GrantType}\" is not supported.");
    }

    private ForbidResult GetForbidResult(string errorDescription,
        string error = Errors.InvalidGrant)
    {
        ...
        ApplicationUser user, IEnumerable<string> scopes)
    {
        ...
        if (user.JobTitle != null)
            identity.SetClaim(PropertyConstants.JobTitle, user.JobTitle);
        if (user.FullName != null)
            identity.SetClaim(PropertyConstants.FullName, user.FullName);
        if (user.Configuration != null)
            identity.SetClaim(PropertyConstants.Configuration,
                user.Configuration);
