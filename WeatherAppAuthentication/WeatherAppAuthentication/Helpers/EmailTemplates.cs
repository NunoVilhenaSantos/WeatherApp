...
public static class EmailTemplates
{
    ...
    {
        testEmailTemplate ??=
            ReadPhysicalFile("Helpers/Templates/TestEmail.template");
    ...
    {
        plainTextTestEmailTemplate ??=
            ReadPhysicalFile("Helpers/Templates/PlainTextTestEmail.template");
    ...
    private static string ReadPhysicalFile(string path)
    {
        if (_hostingEnvironment == null)
            throw new InvalidOperationException(
                $"{nameof(EmailTemplates)} is not initialized");

        var fileInfo =
            _hostingEnvironment.ContentRootFileProvider.GetFileInfo(path);

        if (!fileInfo.Exists)
            throw new FileNotFoundException(
                $"Template file located at \"{path}\" was not found");
