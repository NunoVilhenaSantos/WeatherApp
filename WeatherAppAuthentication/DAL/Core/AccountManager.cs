...
public class AccountManager : IAccountManager
{
        ...
        IHttpContextAccessor httpAccessor)
    {
        ...
        _context.CurrentUserId = httpAccessor.HttpContext?.User
            .FindFirst(ClaimConstants.Subject)?.Value?.Trim();
    ...
    public async Task<(ApplicationUser User, string[] Roles)?>
        GetUserAndRolesAsync(string userId)
    {
    ...
    public async Task<List<(ApplicationUser User, string[] Roles)>>
        GetUsersAndRolesAsync(int page, int pageSize)
    {
        ...
        var userRoleIds = users.SelectMany(u => u.Roles.Select(r => r.RoleId))
            .ToList();
        ...
        return users
            .Select(u => (u,
                roles.Where(r => u.Roles.Select(ur => ur.RoleId).Contains(r.Id))
                    .Select(r => r.Name).ToArray()))
            .ToList();
    }

    public async Task<(bool Succeeded, string[] Errors)> CreateUserAsync(
        ApplicationUser user, IEnumerable<string> roles, string password)
    {
        ...
        ApplicationUser user, string newPassword)
    {
        ...
        var result =
            await _userManager.ResetPasswordAsync(user, resetToken,
                newPassword);
        if (!result.Succeeded)
            return (false, result.Errors.Select(e => e.Description).ToArray());
    ...
    public async Task<bool> CheckPasswordAsync(ApplicationUser user,
        string password)
    {
    ...
    public async Task<(bool Succeeded, string[] Errors)> DeleteUserAsync(
        string userId)
    {
    ...
    public async Task<List<ApplicationRole>> GetRolesLoadRelatedAsync(int page,
        int pageSize)
    {
    ...
    public async Task<(bool Succeeded, string[] Errors)> CreateRoleAsync(
        ApplicationRole role, IEnumerable<string> claims)
    {
        claims ??= new string[] { };

        var invalidClaims = claims.Where(c =>
            ApplicationPermissions.GetPermissionByValue(c) == null).ToArray();
        if (invalidClaims.Any())
            return (false,
                new[]
                {
                    $"The following claim types are invalid: {string.Join(", ", invalidClaims)}"
                });
        ...
        foreach (var claim in claims.Distinct())
        {
            result = await _roleManager.AddClaimAsync(role,
                new Claim(ClaimConstants.Permission,
        ...
        ApplicationRole role, IEnumerable<string> claims)
    {
        ...
        {
            var invalidClaims = claims.Where(c =>
                    ApplicationPermissions.GetPermissionByValue(c) == null)
                .ToArray();
            if (invalidClaims.Any())
                return (false,
                    new[]
        ...
        {
            var roleClaims =
                (await _roleManager.GetClaimsAsync(role)).Where(c =>
            ...
            var claimsToAdd =
                claims.Except(roleClaimValues).Distinct().ToArray();
    ...
    {
        return !await _context.UserRoles.Where(r => r.RoleId == roleId)
            .AnyAsync();
    }
